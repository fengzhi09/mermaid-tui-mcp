...
Project-scoped servers are typically committed to the repo so every contributor gets the same toolset. Because the server runs through `npx`, no local path or clone is required — `npx` resolves the package from the npm registry on first use and caches it.
...
To stop it, `Ctrl-C` the shell. For a managed background daemon (PID file + log + auto-restart) you can also use the bash helper shipped in the npm package at `node_modules/mermaid-tui-mcp/bin/start.sh` — but for most use cases the inline command above is enough.
