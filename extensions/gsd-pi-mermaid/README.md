...
`mermaid-tui-mcp` is published on npm as [`mermaid-tui-mcp`](https://www.npmjs.com/package/mermaid-tui-mcp). The `mermaid-direct` gsd-pi extension ships inside the npm package at `node_modules/mermaid-tui-mcp/extensions/gsd-pi-mermaid/`. Two install paths:
...
Path B uses the dev default for `MERMAID_SERVER_PATH` (resolves to `<projectRoot>/src/server.mjs`, where `projectRoot` is `PI_PROJECT_DIR` or `process.cwd()`), so you only need to set `MERMAID_SERVER_PATH` explicitly if you are running gsd-pi from a directory other than the `mermaid-tui-mcp` repo root.
...
gsd-pi from a project that already has `mermaid-tui-mcp` as a dep
...
key — see gsd-pi docs. The `mermaid-tui-mcp` npm package is now the
